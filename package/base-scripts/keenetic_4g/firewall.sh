...
eval `flash OP_MODE PPP_TYPE WAN_IP_ADDRESS_MODE DMZ_HOST URLFILTER_ENABLED IPFILTER_MODE \
PORTFILTER_ENABLED MACFILTER_MODE PORTFW_ENABLED DMZ_ENABLED LAN_IP_ADDR WAN_TTL_INC_ENABLED \
LAN_SUBNET_MASK LAN_IP_ADDR WAN_AUTO_QOS_ENABLED DHCP_O60_TBL_NUM`
...
set_management_access() {
	eval `flash FTP_ACCESS_ENABLED FTP_WAN_ACCESS_ENABLED FTP_PORT \
...
set_ping_access() {
	...
	if [ "$WAN_PING_ENABLED" = 'Disabled' ]; then
		...
		if [ "$PPP_TYPE" != 'None' ] && [ -n "$PHY_WAN" ]; then
			iptables -A INPUT -p icmp --icmp-type echo-request -i $PHY_WAN  -j DROP
		fi
	fi
...
set_tos() {
	...
	# iptables -A OUTPUT -t mangle -m owner --cmd-owner transmission -j CONNMARK --set-mark 5
